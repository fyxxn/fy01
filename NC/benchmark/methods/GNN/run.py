...
def run_model_DBLP(args):
    ...
    for _ in range(args.repeat):
        ...
        # training loop
        net.train()
        early_stopping = EarlyStopping(patience=args.patience, verbose=True, save_path='/public/home/zlj/HGB_autobenchmark/NC/benchmark/methods/GNN/checkpoint/checkpoint_{}_{}.pt'.format(args.dataset, args.model_type))
        ...
        net.load_state_dict(torch.load('/public/home/zlj/HGB_autobenchmark/NC/benchmark/methods/GNN/checkpoint/checkpoint_{}_{}.pt'.format(args.dataset, args.model_type)))
        net.eval()
        test_logits = []
        with torch.no_grad():
            logits,embedding = net(features_list)
            test_logits = logits[test_idx]
            pred = test_logits.cpu().numpy().argmax(axis=1)
            onehot = np.eye(num_classes, dtype=np.int32)
            dl.gen_file_for_evaluate(test_idx=test_idx,label=pred, file_name=f"/public/home/zlj/HGB_autobenchmark/NC/benchmark/methods/GNN/checkpoint/{args.dataset}.txt")
            ...
            X = embedding[test_idx].cpu().numpy()
            np.save("embedding_{}_gat.npy".format(args.dataset),X)
            np.save("color_{}_gat.npy".format(args.dataset),color)
            print(X.shape)
            print(color.shape)
