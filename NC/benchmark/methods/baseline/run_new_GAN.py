...
from model.GNN_GAN import myGAT
import dgl
...
def run_model_DBLP(args):
    ...
    for _ in range(args.repeat):
        ...
        # training loop
        # net.train()
        early_stopping = EarlyStopping(patience=args.patience, verbose=True, save_path='./NC/benchmark/methods/baseline/checkpoint_GAN/checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers))
        for epoch in range(args.epoch):
            # ---------------------------------------------------------------------------------------------------------------------------
            ''' 训练生成器'''
            for ele in net.named_parameters():
                name = ele[0]
                if 'disc' in name:
            ...
            net.train()
            logits,middle_src,middle_bu= net(features_list,e_feat,feat_drop_idx,src_node_type,mask_list,feat_keep_idx,device,emb,type_mask,args)
            logp = F.log_softmax(logits, 1)
            loss_classification = F.nll_loss(logp[train_idx], labels[train_idx])
            fake_logits_ae = net.disc(middle_bu).reshape([-1])
            G_lbls_1 = torch.ones_like(fake_logits_ae)
            G_loss_ae = BCE(fake_logits_ae, G_lbls_1).mean()
            G_loss = args.loss_lambda*G_loss_ae 
            train_loss = loss_classification +  G_loss
            # auto grad
            optimizer.zero_grad()
            train_loss.backward()
            optimizer.step()
            ...
            logp = F.log_softmax(logits, 1)
            loss_classification = F.nll_loss(logp[train_idx], labels[train_idx])
            true_logits_ae = net.disc(middle_src).reshape([-1])
            fake_logits_ae = net.disc(middle_bu).reshape([-1])
            ...
            with torch.no_grad():
                ...
                val_loss = loss_classification 

            t_end = time.time()
            # print validation info
            print('Epoch {:05d} | Val_Loss {:.4f}| Time(s) {:.4f}'.format(
                epoch, val_loss.item(), loss_classification.item(),t_end - t_start))
            # early stopping
        ...
        net.load_state_dict(torch.load('./NC/benchmark/methods/baseline/checkpoint_GAN/checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers)))
        net.eval()
        test_logits = []
        with torch.no_grad():
            logits,middle_src,middle_bu= net(features_list,e_feat,feat_drop_idx,src_node_type,mask_list,feat_keep_idx,device,emb,type_mask,args)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--loss-d', type=float, default=3, help='Coefficient lambda to balance loss.')
