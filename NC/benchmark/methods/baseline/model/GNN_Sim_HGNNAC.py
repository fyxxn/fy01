...
class myGAT(nn.Module):
    ...
    def forward(self, adj,features_list,e_feat,feat_drop_idx,node_type_src,mask_list,feat_keep_idx,device,emb,type_mask,args):
        ...
        for fc, feature in zip(self.fc_list, features_list):
            transformed_features.append(fc(feature))
        transformed_features = torch.cat(transformed_features, 0)
        #----------------------------------------------------------------------------------------------
        feat_src = transformed_features

        feature_src_re = self.hgnn_ac(adj[mask_list[node_type_src]][:, mask_list[node_type_src]][:, feat_keep_idx],emb[mask_list[node_type_src]], emb[mask_list[node_type_src]][feat_keep_idx],
                                        feat_src[mask_list[node_type_src]][feat_keep_idx])
        # print(feat_src.shape)
        # print(feature_src_re.shape)
        loss_ac = F.mse_loss(feat_src[mask_list[node_type_src]][feat_drop_idx],
                            feature_src_re[feat_drop_idx],reduction='mean')
