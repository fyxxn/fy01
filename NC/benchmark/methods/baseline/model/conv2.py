...
class GATv2Conv(nn.Module):
    ...
    def __init__(
        ...
        self.attn_fc = nn.Linear(in_feats, num_heads, bias=bias)
    ...
    def forward(self, graph, feat, e_feat, res_attn=None,get_attention=False):
        r"""
        Description
        -----------
        ...
        with graph.local_scope():
            ...
            #ee = (e_feat).sum(dim=-1).unsqueeze(-1)
            # ee = (e_feat * self.attn).sum(dim=-1).unsqueeze(-1)
            ...
            # 使用 u_add_v 函数对 el、er 进行逐元素相加并存储到 'e' 键中
            graph.apply_edges(fn.u_add_v("el", "er", "e"))  

            # 提取 'e' 与 'ee' 张量，并进行逐元素相加，然后通过 LeakyReLU 激活函数处理
            ...
            if get_attention:
                return rst, rst, graph.edata.pop('a').detach()
            else:
                return rst,res_attn
