...
class myGAT(nn.Module):
    def __init__(self,
        ...
        self.disc = Discriminator(num_hidden, num_hidden, dropout_rate)
    ...
    def forward(self, features_list,e_feat,feat_drop_idx,node_type_src,mask_list,feat_keep_idx,device,emb,type_mask,args):
        ...
        for i in range(len(features_list)):
            if i!=node_type_src:          
                ...
                middle_bu.append(feat_src[mask_list[i]])
         
        middle_bu=torch.cat(middle_bu)
        # h = torch.cat(h, 0)
        ...
        return logits,middle_src,middle_bu
