...
def run_model_DBLP(args):
    ...
    for _ in range(args.repeat):
        ...
        # training loop
        net.train()
        early_stopping = EarlyStopping(patience=args.patience, verbose=True, save_path='/home/zlj/code/HGB_autobenchmark/NC/benchmark/methods/baseline/checkpoint/checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers))
        for epoch in range(args.epoch):
            ...
            logits,loss_ac = net(features_list,e_feat,feat_drop_idx,src_node_type,mask_list,feat_keep_idx,device,emb,type_mask)
        ...
        net.load_state_dict(torch.load('/home/zlj/code/HGB_autobenchmark/NC/benchmark/methods/baseline/checkpoint/checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers)))
        net.eval()
        test_logits = []
        with torch.no_grad():
            logits,_ = net(features_list,e_feat,feat_drop_idx,src_node_type,mask_list,feat_keep_idx,device,emb,type_mask)
            test_logits = logits[test_idx]
            pred = test_logits.cpu().numpy().argmax(axis=1)
            onehot = np.eye(num_classes, dtype=np.int32)
            dl.gen_file_for_evaluate(test_idx=test_idx, label=pred, file_name=f"/home/zlj/code/HGB_autobenchmark/NC/benchmark/methods/baseline/{args.dataset}_{args.run}.txt")
            pred = onehot[pred]
