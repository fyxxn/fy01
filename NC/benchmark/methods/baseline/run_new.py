...
def run_model_DBLP(args):
    ...
    for _ in range(args.repeat):
        ...
        net = myGAT(g, args.edge_feats, len(dl.links['count'])*2+1, in_dims, 
            ...
            args.dropout, args.slope, True, 0.05,emb.size()[1],args.hidden_dim,args.num_heads,args.run)
        # net = myGAT(g, args.edge_feats, len(dl.links['count'])*2+1, in_dims, 
        # args.hidden_dim, num_classes, args.num_layers, heads, F.elu, args.dropout, 
        ...
        # training loop
        net.train()
        early_stopping = EarlyStopping(patience=args.patience, verbose=True, save_path='/root/autodl-tmp/HGB_autobenchmark/NC/benchmark/methods/baseline/checkpoint/checkpoint_{}_{}.pt'.format(args.dataset, args.run))
        ...
        net.load_state_dict(torch.load('/root/autodl-tmp/HGB_autobenchmark/NC/benchmark/methods/baseline/checkpoint/checkpoint_{}_{}.pt'.format(args.dataset, args.run)))
        net.eval()
        test_logits = []
        with torch.no_grad():
            ...
            dl.gen_file_for_evaluate(test_idx=test_idx, label=pred, file_name=f"/root/autodl-tmp/HGB_autobenchmark/NC/benchmark/methods/baseline/{args.dataset}_{args.run}.txt")
            pred = onehot[pred]
...
if __name__ == '__main__':
    ...
    # ap.add_argument('--num-heads', type=int, default=4, help='Number of the attention heads. Default is 8.') # 8
