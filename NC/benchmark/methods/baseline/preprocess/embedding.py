...
if args.dataset == 'DBLP':
    ...
    print(f.shape)
    np.save('/home/zlj/code/HGB_autobenchmark/data/DBLP/metapath2vec_emb_node.npy', f)
    ...
    t_feature = np.zeros((T_n, dim), dtype=np.float)
    ...
    if T_counter != T_n:
        print('{} t node miss'.format(T_n - T_counter))

    f = np.concatenate((p_feature, a_feature, s_feature,t_feature), axis=0)
    print(f.shape)
    np.save('/home/zlj/code/HGB_autobenchmark/data/ACM/metapath2vec_emb_node.npy', f)
    print('done!')
    ...
    m_feature = np.zeros((M_n, dim), dtype=np.float)
    d_feature = np.zeros((D_n, dim), dtype=np.float)
    a_feature = np.zeros((A_n, dim), dtype=np.float)

    path = r'metapath2vec_IMDB_embeddings.txt'
    ...
    for line in islice(f, 2, None):
        ...
        node_id = int(node_info[1])
        if node_type == 'mMovie':
            m_feature[node_id] = line[1:]
            M_counter += 1
        if node_type == 'dDirector':
            d_feature[node_id] = line[1:]
            D_counter += 1
        if node_type == 'aActor':
            a_feature[node_id] = line[1:]
            A_counter += 1
    ...
    if M_counter != M_n:
        print('{} p node miss'.format(M_n - M_counter))
    if D_counter != D_n:
        print('{} a node miss'.format(D_n - D_counter))
    if A_counter != A_n:
        print('{} s node miss'.format(A_n - A_counter))


    f = np.concatenate((m_feature, d_feature, a_feature), axis=0)
    print(f.shape)
    
    np.save('/home/zlj/code/HGB_autobenchmark/data/IMDB/metapath2vec_emb_node.npy', f)
    print('done!')
