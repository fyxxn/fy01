...
from model.GNN_Sim_HGNNAC import myGAT
import dgl
...
def run_model_DBLP(args):
    ...
    result_macro_f1=[]
    # print(type(adjM))
    adjM=torch.tensor(adjM.A).to(device)
    for _ in range(args.repeat):
        ...
        # training loop
        net.train()
        early_stopping = EarlyStopping(patience=args.patience, verbose=True, save_path='./NC/benchmark/methods/baseline/checkpoint_sim_hgnnac/checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers))
        for epoch in range(args.epoch):
            ...
            logits,loss_ac = net(adjM,features_list,e_feat,feat_drop_idx,src_node_type,mask_list,feat_keep_idx,device,emb,type_mask,args)
        ...
        net.load_state_dict(torch.load('./NC/benchmark/methods/baseline/checkpoint_sim_hgnnac/checkpoint_{}_{}.pt'.format(args.dataset, args.num_layers)))
        net.eval()
        test_logits = []
        with torch.no_grad():
            logits,_ = net(adjM,features_list,e_feat,feat_drop_idx,src_node_type,mask_list,feat_keep_idx,device,emb,type_mask,args)
